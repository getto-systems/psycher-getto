...
const notify_gitlab_success = require("../../lib/actions/notify_gitlab_success");

test("notify gitlab success", async () => {
  ...
  notify_gitlab_success.perform(slack);

  expect(slack_request_mock.data.reaction.length).toBe(1);
  expect(slack_request_mock.data.reaction[0]).toBe("gitlab-success");
});
