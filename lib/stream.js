...
const init = ({secret_store, message_store}) => {
  ...
  const post = async ({reply_to, text}) => {
    const token = await find_message_token(reply_to);
  ...
  const find_message_token = async (reply_to) => {
    const tokens = await secret_store.message_tokens();
    if (tokens && tokens[reply_to.as]) {
      return tokens[reply_to.as];
    }

    throw "no message token: " + reply_to.as;
  };
